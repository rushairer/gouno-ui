...
| API-031 | Core `Timeline` / `TimelineProps` / `TimelineItem` 从 canonical 公共 API 移除。真实 Blog 同时存在 Markdown timeline document template 与 workflow run event log，但两者都没有消费 Core `Timeline`，且语义/数据结构明显不同：前者是文档内容，后者拥有时间、事件类型、step、payload 与 newest-first 记录顺序。旧 `title/description/items[]` wrapper 使用数组 index key并固定左线/圆点视觉，无法表达这些真实职责。没有独立跨产品证据证明一个统一 Timeline contract 时，不保留或扩展 generic timeline wrapper。 | Core Timeline de-admission invariant + Blog CustomPage/WorkflowRun counter-evidence / PD-055 |
