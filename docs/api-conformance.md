...
| Patterns | active/minimal | 当前准入 `BulkActionBar`、`DocumentEditorShell` 与 `MarkdownEditor`；继续要求跨真实产品证据，不恢复历史 feature bag。 |
...
| API-041 | Core `Cascader` 完成 reviewed 100：使用单一 Gouno-owned combobox trigger + 多列 popup 表达级联路径，`value/defaultValue/onChange` 仍是唯一路径状态；清空第 N 级会截断该级及更深路径。根 group 与可见 trigger 复用 caller-owned `aria-label/aria-labelledby`，popup 各列使用语言无关数字名称；`placeholder` 文案由调用方拥有。可见 UI 不依赖 OS native select，并复用 canonical `ControlSize`、status、Focus、Surface、Elevation 与 popup Layer；搜索、异步加载和业务地址模型仍不进入 Core。 | same-source Showcase/API docs + `core-cascader-6d6` focused tests + picker consistency maintenance / PD-065 superseded by PD-080 |
| API-042 | Core `TreeSelect` 完成 reviewed 100：使用 Gouno-owned combobox trigger + canonical `Tree` popup 表达真实层级结构；single 使用 tree selection，multiple 使用 checkable tree，`value/defaultValue/onChange` 继续接受 string / readonly string[] 并在 multiple 时回传 string[]。隐藏 native select 仅承担 name/required/value/ref 等 form compatibility，不参与可见渲染；`TreeSelectNode.title` 保持 string，`placeholder` caller-owned。可见控制面复用 canonical `ControlSize`、status、Focus、Surface、Elevation 与 popup Layer；搜索、异步树和 custom renderer 仍需独立证据。 | same-source Showcase/API docs + `core-tree-select-6d7` focused tests + picker consistency maintenance / PD-066 superseded by PD-080 |
...
当前重新准入的 Pattern 包括 `BulkActionBar`、`DocumentEditorShell` 与 `MarkdownEditor`。`BulkActionBar` 只拥有批量选择上下文、产品动作插槽、取消入口和 toolbar/sticky 语义；资源列表、选择状态、确认流程、AI/发布/删除等业务动作继续由产品拥有。它不能作为恢复 DataTable、FilterBar 或通用 feature bag 的先例。
