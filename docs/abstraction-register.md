...
### PD-052 — Image is de-admitted after cross-product native-image counter-evidence
- **Status:** accepted / Core de-admission
- **Owner:** Core / public API governance / completed-corpus audit
- **Evidence:** the real Gouno Blog uses native `<img>` across public Home/ArticleTeaser/Markdown/PostDetail and Blog Admin Media Library, Site Settings, PostEditor, PageEditor and AI/media preview surfaces; Gosso Admin LoginSurface independently uses native `<img>` for branding. These products own alt text, lazy loading, object-fit/layout, fallback/error presentation and click/preview behavior directly. No real product consumer imports Core `Image`. The removed wrapper additionally exposed a `preview` prop that only changed the cursor, latched its error state across later `src` changes and injected the English fallback `"Image unavailable"`.
- **Decision:** remove Core `Image` rather than expand or harden an abstraction disproved by the completed product corpus. Ordinary image rendering returns to native HTML ownership. Product-specific fallback, progressive loading, media inspection, zoom/lightbox and other stateful interactions remain product-owned until independent products prove a smaller stable shared contract beyond `<img>`.
- **API impact:** remove `Image` and `ImageProps` from `@gouno/ui/core`; the root compatibility umbrella loses them automatically because it re-exports Core. Remove the Core Image source file, Showcase family/document and runtime-family registration. This is a breaking public API contraction; no compatibility alias is retained.
- **Migration impact:** consumers updating the package replace ordinary `<Image>` usage with native `<img>` and own any localized error/preview state beside the product resource lifecycle. The old `preview` prop has no direct replacement because it never implemented an actual preview interaction.
- **Validation:** `tests/core-image-deadmission.test.ts` protects absence from the canonical Core runtime, family coverage, Data Display docs and Showcase Catalog. Existing Blog/Gosso product evidence remains unchanged on native `<img>`. Acceptance still requires exact-main typecheck, complete tests, package build, Showcase build and `gh-pages` publication.
- **Re-admission rule:** do not recreate a generic Image wrapper merely for library parity or shorter JSX. Re-admission requires new independent cross-product evidence of a product-agnostic behavior contract beyond native HTML, including a defined accessibility/interaction lifecycle when preview or other stateful behavior is involved.
- **Follow-up:** continue auditing unsupported Core breadth with the same keep/harden/de-admit decision test; distinguish components that have real product pressure from components sustained only by Showcase/catalog presence.

### PD-053 — List is de-admitted after completed-product collection counter-evidence
- **Status:** accepted / Core de-admission
- **Owner:** Core / public API governance / completed-corpus audit
- **Evidence:** no completed Gosso Admin, Blog Admin or Blog public product consumer imports Core `List`; repository-wide `renderItem=` usage exists only in the Core Showcase document. Real collection surfaces instead choose semantic native lists, Tables, Cards/grids and product-local responsive compositions according to resource/task needs. The removed bootstrap wrapper additionally injected English `"No data"`, used array indexes as React keys, fixed row `p-3` and defaulted to its own bordered/divided surface.
- **Decision:** remove Core `List` rather than harden a generic data/renderItem feature bag that the completed product corpus has not validated. Native `<ul>/<ol>` and explicit product composition remain the default. Table or another admitted collection primitive is used only when its separate semantics actually match.
- **API impact:** remove `List` and `ListProps` from `@gouno/ui/core` and therefore from the root compatibility umbrella; remove `src/core/list.tsx`, its Showcase Data Display document, catalog entry and runtime-family registration. This is a breaking public API contraction with no alias.
- **Migration impact:** consumers replace simple List usage with semantic native list markup and stable product-owned keys, or choose an already-admitted collection surface whose semantics fit. Empty state content remains explicit via Core `Empty` or product-owned content; no generic English fallback survives.
- **Validation:** `tests/core-list-deadmission.test.ts` protects absence from the canonical runtime, family coverage, Data Display docs and Showcase Catalog. Completed product corpora remain unchanged because none consumed Core `List`.
- **Re-admission rule:** do not recreate `data/renderItem/bordered/empty` just for library parity or shorter mapping syntax. Re-admission requires new independent product evidence for a stable collection behavior that native list semantics plus existing Core composition cannot express cleanly.
- **Follow-up:** continue auditing bootstrap-era Core breadth one family at a time, preferring removal over speculative hardening when completed products provide only counter-evidence.

### PD-054 — Descriptions is de-admitted after native definition-list counter-evidence
- **Status:** accepted / Core de-admission
- **Owner:** Core / public API governance / completed-corpus audit
- **Evidence:** Gosso Account Profile, System Status and Audit Detail, plus Blog Proposal/Agent metadata, already use semantic native `<dl>/<dt>/<dd>` directly. Those independently owned surfaces need different responsive label/value grids, row separators, surrounding Card/Modal ownership, value rendering and stable domain keys. No completed real product consumer imports Core `Descriptions`; its only canonical use was the Core Showcase self-example. The removed wrapper additionally used array indexes as keys, limited layout to fixed `columns=1|2|3`, and coupled definition-list semantics to an optional built-in border/padding surface.
- **Decision:** remove Core `Descriptions` rather than harden a generic `items/columns/bordered` wrapper that real products have already bypassed in favor of platform semantics and explicit composition. Native definition lists remain the default; products own layout and surrounding surface because those decisions follow the actual information hierarchy.
- **API impact:** remove `Descriptions` and `DescriptionsProps` from `@gouno/ui/core` and therefore from the root compatibility umbrella; remove `src/core/descriptions.tsx`, its Showcase Data Display document, Catalog entry and runtime-family registration. This is a breaking public API contraction with no alias.
- **Migration impact:** consumers replace simple `Descriptions` usage with `<dl>/<dt>/<dd>`, stable product-owned keys and local responsive layout. Border/divider/Card presentation stays with the owning surface rather than a generic definition-list wrapper.
- **Validation:** `tests/core-descriptions-deadmission.test.ts` protects absence from canonical runtime, family coverage, Data Display docs and Showcase Catalog. Existing Gosso/Blog native definition-list product evidence remains unchanged.
- **Re-admission rule:** do not recreate `items/columns/bordered` for library parity or to shorten markup. Re-admission requires independent cross-product evidence of a stable behavior beyond native definition-list semantics and existing Core composition.
- **Follow-up:** continue auditing bootstrap-era Core breadth one family at a time; do not infer that every data-display family should be removed without its own completed-corpus evidence review.
