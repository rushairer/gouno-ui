...
### PD-063 — Mentions preserves multiline textbox semantics while owning only local suggestion mechanics

- **Status:** accepted / Core hardening
- **Owner:** Core / Data Entry family
- **Evidence:** `Mentions` is an established retained Core control, but its pre-6D4 implementation always stored `defaultValue` internally even when consumers supplied `value`, accepted only mouse selection, exposed no real textarea ref or suggestion relationship, and built its trigger regex directly from `prefix`. It also reused the native textarea `onSelect` prop name for mention confirmation without making that ownership explicit.
- **Decision:** preserve the semantic element as a native multiline textarea/textbox rather than overriding it with a single-line combobox role. `value/defaultValue/onChange(string)` is the single text-state contract. Core owns only the active trailing mention token, literal-prefix escaping, local case-insensitive option filtering, textbox-to-listbox association, ArrowUp/ArrowDown highlight, Enter confirmation and Escape dismissal. Home/End remain native multiline editing keys. Caller focus/blur/key handlers compose before internal behavior and may stop it with `preventDefault`.
- **API impact:** `MentionsProps` is now owned directly by `mentions.tsx` and exported from the canonical Core entrypoint. It reuses Textarea `ControlSize`, `status`, `showCount`, `maxLength`, standard form/ARIA/data/event attributes and real textarea ref. React's native text-selection `onSelect` is intentionally omitted because `onSelect(value)` has one component-level meaning: confirmed mention selection. No non-standard accessible-name alias is added.
- **Scope boundary:** no remote lookup, debounce, async loading state, user/resource fetching, rich mention chips, multi-prefix object configuration or product-domain suggestion rendering is admitted from this batch.
- **Validation:** 6D4 runtime/same-source Showcase/focused tests landed at `main@c4a645f`. Exact-head run 313 found one stale legacy test that expected a popup from an unfocused textarea; the test was aligned to the real focus contract at `main@a8f6f93`, and run 314 then passed the full gate. Props ownership landed at `main@9bdd916`; run 315 passed typecheck, all tests, package build, Showcase build, pack, artifact upload and Pages publication.
- **Abstraction impact:** no typeahead/search/community Pattern is admitted. Mentions remains a Core multiline text-entry primitive; product lookup and identity semantics stay caller-owned.
...
### PD-078 — Dedicated Editor is a Showcase composition contract, not a public runtime component

- **Status:** accepted / private composition contract
- **Owner:** Product interface governance + Showcase Patterns
- **Evidence:** Blog Admin independently contains three deep Configuration Editor cases (Agent, Skill and Workflow) plus two sustained Workspace Editor cases (Post and Page). Agent/Skill previously stacked a TabPanel lead above a second EditorHeader, while Workflow used a different single-lead composition. All five cases need consistent task identity, explicit return, feedback ownership, responsive editor geometry and entry scroll behavior, but their runtime bodies and business state differ materially.
- **Decision:** define Dedicated Editor as a canonical Showcase composition pattern with two subtypes: Configuration Editor and Workspace Editor. Configuration Editor uses one canonical lead, canonical section/layout rhythm and one action boundary. Workspace Editor may use a purpose-built shell such as DocumentEditorShell while retaining the same upper-level task responsibilities. Showcase-private helpers encode the Configuration composition for product fixtures.
- **Public API impact:** none. Do not export a `DedicatedEditor` component from `src/patterns`. Repetition of page composition is not sufficient evidence for a slot-heavy public runtime API. Existing `DocumentEditorShell` remains the admitted runtime Pattern for the narrower workspace-shell behavior it actually owns.
