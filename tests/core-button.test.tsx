...
describe("Core Button", () => {
  it("renders variants, sizes, shapes, block, icons and loading state", () => {
    render(
      <>
        <Button variant="dashed" size="large" shape="round" block icon={<ArrowRight />}>
          Continue
        </Button>
  ...
  it("maps semantic colors to canonical utility classes instead of dead migration hooks", () => {
    render(
      <>
        ...
        <ButtonLink href="/info" color="info">
          Info
        </ButtonLink>
      </>,
    );
  ...
  it("renders navigation as a real link and prevents disabled navigation", () => {
    ...
    render(
      <>
        <ButtonLink href="/external">External</ButtonLink>
        <ButtonLink href="/disabled" disabled onClick={onClick}>
          Disabled
        </ButtonLink>
      </>,
    );
    ...
    expect((disabled as HTMLAnchorElement).tabIndex).toBe(-1);
  ...
  it("uses a router adapter through NavigationProvider", () => {
    render(
      <NavigationProvider link={RouterLink}>
        <ButtonLink to="/docs" variant="solid" color="primary">
          Docs
        </ButtonLink>
      </NavigationProvider>,
    );
    expect(screen.getByRole("link", { name: "Docs" }).getAttribute("data-route")).toBe(
      "/docs",
    );
  });

  it("centers a single IconButton icon without rendering an empty label span", () => {
  ...
  it("keeps IconButtonLink icon-only while preserving real-link semantics", () => {
    render(<IconButtonLink label="Next page" icon={<ArrowRight />} href="/next" />);
  ...
  it("maps ChoiceButton selection to aria-pressed without owning product state", () => {
    render(
      <>
        ...
        <ChoiceButton selected={false}>Code</ChoiceButton>
      </>,
    );
    expect(screen.getByRole("button", { name: "Preview" }).getAttribute("aria-pressed")).toBe(
      "true",
    );
    expect(screen.getByRole("button", { name: "Code" }).getAttribute("aria-pressed")).toBe(
      "false",
    );
  });
});
