...
function renderIndex(
  ...
  return render(
    <ThemeProvider
      brand="blog"
      storageKey={`blog-article-index-${mode}-theme`}
    >
...
describe("Blog public ArticleIndex product migration fixture", () => {
  it("preserves article listing, filters and canonical Pagination behavior", () => {
    ...
    expect(
      screen.getByRole("heading", {
        name: /Kafka 15 分区为什么不是 15 个 goroutine 就最快/,
      }),
    ).toBeTruthy();
  });
  ...
  it("keeps Search as a route intent of the same ArticleIndex implementation", () => {
    ...
    expect(
      screen.getByText("2 篇文章，持续记录问题、选择与实现。"),
    ).toBeTruthy();
  ...
  it("preserves loading, empty and error states without a real Blog API", () => {
    ...
    expect(
      screen.getByRole("status", { name: "文章列表加载中" }),
    ).toBeTruthy();
    loading.unmount();
  ...
  it("keeps discovery composition product-local and route links semantic", () => {
