...
describe("Core data entry controls", () => {
  ...
  it("supports autocomplete arrow navigation and Enter selection", () => { const onChange = vi.fn(); render(<AutoComplete aria-label="City" options={["Beijing", "Berlin"]} onChange={onChange} />); const input = screen.getByRole("combobox"); fireEvent.focus(input); fireEvent.keyDown(input, { key: "ArrowDown" }); fireEvent.keyDown(input, { key: "Enter" }); expect(onChange).toHaveBeenLastCalledWith("Berlin"); });
  it("supports controlled tree selection and disabled nodes", () => { const onChange = vi.fn(); render(<TreeSelect aria-label="Section" treeData={[{ value: "docs", title: "Docs", children: [{ value: "api", title: "API", disabled: true }] }]} value="docs" onChange={onChange} />); const trigger = screen.getByRole("combobox", { name: "Section" }); expect(trigger.textContent).toContain("Docs"); fireEvent.click(trigger); expect(screen.getByRole("treeitem", { name: /API/ }).getAttribute("aria-disabled")).toBe("true"); fireEvent.click(screen.getByRole("treeitem", { name: /Docs/ })); expect(onChange).toHaveBeenLastCalledWith("docs"); });
