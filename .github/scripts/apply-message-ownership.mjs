...
index = replaceRequired(
  ...
  "MessageProvider direct export",
);
index = replaceRequired(
  index,
  '  MessageProviderProps,\n',
  "",
  "MessageProvider public-props re-export",
);
writeFileSync(indexPath, index);

copyFileSync(".github/scripts/package.original.json", "package.json");
unlinkSync(".github/scripts/package.original.json");
unlinkSync(".github/scripts/apply-message-ownership.mjs");
...
execFileSync("git", ["commit", "-m", "refactor(core): let MessageProvider own its public props"], { stdio: "inherit" });
