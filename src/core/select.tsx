  ...
  return <div className="relative min-w-0" data-slot="select">
    ...
    <Popover open={open} onOpenChange={(next) => { if (!disabled && !loading) setOpen(next); }}>
      <PopoverAnchor asChild>
        <div data-slot="select-control" className={cn("flex w-full min-w-0 items-center gap-2 rounded-md border border-border bg-input px-3 text-foreground focus-within-owner", controlSizeClass(size), multi && "h-auto min-h-9 py-1", status === "error" && "border-destructive focus-within:ring-destructive", status === "warning" && "border-warning focus-within:ring-warning", (disabled || loading) && "cursor-not-allowed opacity-50", className)}>
          ...
          {canClear ? <button type="button" aria-label={text.clearLabel} className="flex size-5 shrink-0 items-center justify-center rounded text-muted-foreground outline-none hover:bg-muted focus-control" onClick={clear}><X aria-hidden="true" className="size-3.5" /></button> : null}
        </div>
      </PopoverAnchor>
      <PopoverContent role="presentation" placement="bottom-start" className="w-[var(--radix-popover-trigger-width)] min-w-48 p-1" onOpenAutoFocus={(event) => { event.preventDefault(); if (searchable) searchRef.current?.focus(); else triggerRef.current?.focus(); }} onCloseAutoFocus={(event) => { event.preventDefault(); triggerRef.current?.focus(); }}>
        ...
        }} placeholder={text.searchPlaceholder} aria-label={text.searchLabel} className="h-8 w-full rounded border border-border bg-input px-2 text-sm outline-none focus-control" /></div> : null}
