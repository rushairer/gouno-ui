...
export function Drawer({
  ...
  height,
  loading = false,
  mask = true, zIndex = 50, destroyOnClose = true, extra, styles,
}: DrawerProps) {
  ...
  return (
    ...
    <Sheet open={visible} onOpenChange={changeOpen}>
      <SheetContent
        ...
        style={{ ...dimensionStyle, maxWidth: "100vw", maxHeight: "100dvh", zIndex, ...contentStyle }}
        mask={mask}
        zIndex={zIndex}
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-auto p-5" style={styles?.body} aria-busy={loading || undefined}>
        ...
        {footer != null && <div className="flex flex-wrap justify-end gap-3 border-t p-5" style={styles?.footer}>{footer}</div>}
      </SheetContent>
    </Sheet>
    </>
  );
