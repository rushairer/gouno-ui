...
export const DatePicker = forwardRef<HTMLInputElement, DatePickerProps>(function DatePicker({ locale, value, defaultValue = "", size = "middle", status, allowClear = true, onChange, className, disabled, readOnly, ...props }, ref) {
  ...
  return <span data-slot="date-picker" data-status={status} className={cn("relative inline-flex w-full min-w-0 items-center rounded-md border border-border bg-input focus-within-owner", status === "error" && "border-destructive", status === "warning" && "border-warning", disabled && "opacity-50", className)}>
