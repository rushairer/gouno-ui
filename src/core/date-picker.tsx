...
export const DatePicker = forwardRef<HTMLInputElement, DatePickerProps>(function DatePicker({ value, defaultValue = "", size = "middle", status, allowClear = true, onChange, className, disabled, readOnly, ...props }, ref) {
  ...
  return <span data-slot="date-picker" data-status={status} className={cn("relative inline-flex w-full min-w-0 items-center rounded-md border border-border bg-input focus-within:ring-2 focus-within:ring-ring", status === "error" && "border-destructive", status === "warning" && "border-warning", disabled && "opacity-50", className)}>
    ...
    {allowClear && current && !disabled && !readOnly ? <button type="button" aria-label="Clear date" className="absolute right-2 flex size-6 items-center justify-center rounded text-muted-foreground hover:bg-muted hover:text-foreground" onClick={() => update("")}><X className="size-3.5" /></button> : null}
  </span>;
});
