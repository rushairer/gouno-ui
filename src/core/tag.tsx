...
export function Tag({ color = "default", icon, bordered = true, closable, closeIcon, onClose, checkable, checked, defaultChecked = false, onChange, disabled, children, className, style, ...props }: TagProps) {
  ...
  const classes = cn("inline-flex min-h-6 items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium", bordered && "border", semanticColor ? colorClass[color] : "text-white", checkable && "cursor-pointer transition-colors focus-standalone", checkable && selected && "border-primary bg-primary text-primary-foreground", disabled && "pointer-events-none opacity-50", className);
