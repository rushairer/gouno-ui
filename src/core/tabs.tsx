...
export function Tabs<T extends string = string>({ items, ariaLabel, tabClassName, children, onValueChange, ...props }: Omit<ComponentProps<typeof Primitive.Tabs>, "onValueChange"> & { items?: readonly TabItem<T>[]; ariaLabel?: string; tabClassName?: string; onValueChange?: (value: T) => void }) {
  return <Primitive.Tabs {...props} onValueChange={value => onValueChange?.(value as T)}>{items ? <Primitive.TabsList aria-label={ariaLabel} className={cn("h-auto max-w-full justify-start overflow-x-auto rounded-none border-b bg-transparent p-0", tabClassName)}>{items.map((item, index) => <Primitive.TabsTrigger key={item.value} value={item.value} onKeyDown={(event: KeyboardEvent<HTMLButtonElement>) => { if (event.key !== "ArrowRight" && event.key !== "ArrowLeft") return; event.preventDefault(); const nextIndex = event.key === "ArrowRight" ? (index + 1) % items.length : (index - 1 + items.length) % items.length; const next = items[nextIndex]; onValueChange?.(next.value); event.currentTarget.parentElement?.querySelectorAll<HTMLElement>('button[role="tab"]')[nextIndex]?.focus(); }} className="gap-2 rounded-none border-b-2 border-transparent px-3 py-3 shadow-none data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:text-primary">{item.icon}{item.label}</Primitive.TabsTrigger>)}</Primitive.TabsList> : null}{children}</Primitive.Tabs>;
}
