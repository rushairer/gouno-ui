...
export const dataDisplayDocuments: Record<string, ComponentDocument> = {
  list: {
    ...
    api: [
      ...
      {
        name: "itemLayout",
        description: "条目内容排列方向",
        type: '"horizontal" | "vertical"',
        defaultValue: '"horizontal"',
      },
      {
        name: "loading",
        description: "用 Core Spin 覆盖列表内容区域",
        type: "boolean",
        defaultValue: "false",
      },
      ...
      {
        name: "loadMore",
        description: "列表后的显式加载更多区域",
        type: "ReactNode",
      },
      {
        name: "locale",
        description: "列表局部文案，当前包含 emptyText",
        type: "ListLocale",
      },
      {
        name: "classNames",
        description: "按语义槽追加 className，可读取当前组件状态",
        type: "ListClassNames",
      },
      {
        name: "styles",
        description: "按语义槽追加 style，可读取当前组件状态",
        type: "ListStyles",
      },
      {
        name: "ref",
        description: "真实根 div 引用",
    ...
    apiSections: [
      ...
      },
      {
        title: "Semantic DOM",
        description:
          "classNames/styles 支持稳定语义槽，不要求调用方依赖内部 DOM 层级。",
        rows: [
  ...
  descriptions: {
    ...
    api: [
      ...
      {
        name: "items",
        description: "带稳定 key 的描述项",
        type: "readonly DescriptionsItem[]",
        defaultValue: "[]",
      },
      {
        name: "bordered",
        description: "显示边框和标签底色",
        type: "boolean",
        defaultValue: "false",
      },
      {
        name: "colon",
        description: "横向布局标签后显示冒号",
        type: "boolean",
        defaultValue: "true",
      },
      {
        name: "column",
        description: "列数或响应式列数映射",
        type: "DescriptionsColumn",
        defaultValue: "3",
      },
      {
        name: "layout",
        description: "标签与内容的排列方式",
        type: '"horizontal" | "vertical"',
        defaultValue: '"horizontal"',
      },
      {
      ...
      {
        name: "classNames",
        description: "按语义槽追加 className",
        type: "DescriptionsClassNames",
      },
      {
        name: "styles",
        description: "按语义槽追加 style",
    ...
    apiSections: [
      {
        ...
        rows: [
          ...
          {
            name: "span",
            description: "跨列数量、filled 或响应式映射",
  ...
  image: {
    ...
    api: [
      {
        name: "fallback",
        description: "主图片加载失败后尝试的备用图片 URL",
        type: "string",
      },
      {
        name: "placeholder",
        description: "图片尚未完成加载时的占位内容",
        type: "ReactNode",
      },
      {
        name: "preview",
        description: "启用预览或配置预览行为",
        type: "boolean | ImagePreviewConfig",
        defaultValue: "true",
      },
      {
        name: "classNames",
        description: "按图片语义槽追加 className",
        type: "ImageClassNames",
      },
      {
        name: "styles",
        description: "按图片语义槽追加 style",
        type: "ImageStyles",
      },
      {
        name: "ref",
        description: "真实缩略图 img 引用",
        ...
        rows: [
          {
            ...
            description: "预览的受控/非受控打开状态",
            type: "boolean",
          },
          {
            name: "src",
            description: "预览层使用的替代图片地址",
            type: "string",
          },
          {
            name: "cover",
            description: "缩略图 hover/focus 预览覆盖层",
            type: "ReactNode | ImageCoverConfig",
          },
          {
            name: "mask",
            description: "遮罩显示、模糊与点击关闭策略",
            type: "boolean | ImagePreviewMaskConfig",
          },
          {
          ...
          {
            name: "wheel",
            description: "允许鼠标滚轮缩放",
            type: "boolean",
            defaultValue: "true",
          },
          {
            name: "onOpenChange",
            description: "预览打开状态变化回调",
            type: "(open: boolean) => void",
          },
          {
            name: "onTransform",
            description: "缩放、旋转、翻转、拖动等变换回调",
            type: "(info: { transform: ImageTransform; action: ImageTransformAction }) => void",
          },
          {
            name: "actionsRender",
            description: "替换或包裹默认工具栏",
            type: "(originalNode: ReactElement, info: ImageToolbarInfo) => ReactNode",
          },
          {
            name: "imageRender",
            description: "替换或包裹预览图片节点",
            type: "(originalNode: ReactElement, info: { transform: ImageTransform; src: string; alt: string }) => ReactNode",
          },
          {
            name: "classNames",
            description: "预览 root/image/toolbar 类名",
            type: 'Partial<Record<"root" | "image" | "toolbar", string>>',
          },
          {
            name: "styles",
            description: "预览 root/image/toolbar/mask 样式",
            type: 'Partial<Record<"root" | "image" | "toolbar" | "mask", CSSProperties>>',
          },
        ],
      },
      {
        ...
        rows: [
          ...
          {
            name: "rotate",
            description: "每次向左/右旋转 90°",
            type: '"rotateLeft" | "rotateRight"',
          },
          {
            name: "zoom",
            description: "按钮、滚轮和双击缩放",
            type: '"zoomIn" | "zoomOut" | "wheel" | "doubleClick"',
          },
          {
            name: "move / reset",
            description: "放大后拖动并可恢复默认变换",
            type: '"move" | "reset"',
          },
        ],
      },
  ...
  statistic: {
    ...
    description:
      "展示一个调用方拥有的指标标签与数值；组件只负责基础排版，不拥有 Card、趋势或格式化策略。",
    ...
    api: [
      ...
      {
        name: "aria-label",
        description: "需要为整个指标组提供额外名称时使用的标准 ARIA 属性",
        type: "string",
      },
      {
        name: "ref",
