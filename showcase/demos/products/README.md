...
The next high-value Blog Admin target is **Dashboard `/admin/dashboard`**. It is a read-dominant operational surface with metric cards, traffic trend, content-health summaries, top-post data and AI failure alerts. Migrate it Core-first and use it to test dashboard composition before considering any `MetricCard`, chart wrapper or dashboard-specific Pattern.
