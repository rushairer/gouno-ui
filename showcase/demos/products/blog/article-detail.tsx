...
import { BlogArticleCommunity } from "./article-community";

type ArticleDetailScenario =
  "data" | "preview" | "loading" | "error" | "not-found";
...
const tocItems = [
  ...
  {
    key: "browser-session",
    title: <span className="pl-3">浏览器只持有本域会话</span>,
  },
  { key: "reading-contract", title: "把安全约束翻译成阅读体验" },
  {
    key: "single-source-code",
    title: <span className="pl-3">代码展示也需要单一来源</span>,
  },
...
function renderSyntaxPreview(source: string) {
  const tokens = source.split(
    /(export|async|function|const|if|return|await|true|false|null)/g,
  );
  return tokens.map((token, index) =>
    /^(export|async|function|const|if|return|await|true|false|null)$/.test(
      token,
    ) ? (
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-5 gap-y-3 border-y py-4 type-body-sm text-muted-foreground">
      ...
      <span className="inline-flex items-center gap-1.5">
        ...
        {articleMeta.views.toLocaleString()} 阅读
      </span>
      <span className="inline-flex items-center gap-1.5">
      ...
      <section aria-labelledby="reading-contract" className="space-y-4">
        ...
        <div className="overflow-x-auto rounded-lg border">
          <table className="w-full min-w-[560px] border-collapse text-left type-body-sm">
            ...
            <tbody>
              <tr>
                <td className="border-b px-4 py-3">固定头部遮挡标题</td>
                <td className="border-b px-4 py-3">scroll-margin-top</td>
                <td className="border-b px-4 py-3">
                  Heading / product content
                </td>
              </tr>
              <tr>
                <td className="border-b px-4 py-3">目录导航</td>
                <td className="border-b px-4 py-3">真实 hash 链接</td>
                <td className="border-b px-4 py-3">Core Anchor</td>
              </tr>
              <tr>
                <td className="px-4 py-3">代码复制</td>
                <td className="px-4 py-3">单一源码 + clipboard feedback</td>
                <td className="px-4 py-3">Core CodeBlock</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export function BlogArticleDetailDemo({
  ...
  const [scenario, setScenario] =
    useState<ArticleDetailScenario>(initialScenario);
  const [notice, setNotice] = useState("");
  const route =
    scenario === "preview"
      ? `/articles/${article.slug}?preview=1`
  ...
  return (
    <div className="relative">
      <FixtureDock
        route={route}
        note="真实 PostDetail 阅读骨架的静态迁移：保留文章层级、元信息、TOC 深链接、代码复制、媒体、相关阅读、讨论区和主要加载/错误/未找到/管理员预览状态；社区交互保持 Blog-owned composition。"
        controls={
      ...
      <BlogPublicShellFixture currentPath={route} onNavigate={navigate}>
        ...
        {notice ? (
          <Alert className="mb-6" type="info" description={notice} showIcon />
        ) : null}
        {scenario === "preview" ? (
          <Alert
            className="mb-6"
