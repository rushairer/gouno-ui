...
type DiscoveryScenario = "data" | "loading" | "empty" | "error";
...
const pageMeta: Record<
  DiscoveryPage,
  { route: string; title: string; meta: string; subtitle: string }
> = {
  categories: {
    route: "/categories",
    title: "分类",
...
function NavigationLink({
  ...
  return (
    <a href={target} className={className} onClick={activate}>
      {children}
    </a>
  );
}
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {blogCategories.map((item, index) => {
        const target = `/categories/${encodeURIComponent(item.slug)}`;
        return (
          <NavigationLink
            key={item.id}
            target={target}
...
export function BlogDiscoveryIndexDemo({
  ...
  return (
    <div className="relative">
      <FixtureDock
        route={meta.route}
        note={`真实 Blog ${meta.title}索引页的静态迁移：复用 PublicShell 与 PageHeader，显式区分加载、失败、空和数据状态，不调用真实 Blog API。`}
        controls={
      ...
      <BlogPublicShellFixture currentPath={meta.route} onNavigate={navigate}>
        <div className="mx-auto flex w-full max-w-[1100px] flex-col gap-8">
          ...
          <PageHeader
            title={meta.title}
            description={
              <PageDescription meta={meta.meta}>{meta.subtitle}</PageDescription>
            }
          />
          <Card aria-label={`${meta.title}内容`}>
            {scenario === "loading" ? (
              ...
              <Alert
                type="error"
                title={`${meta.title}加载失败`}
                description="公开内容服务暂时不可用，请稍后重试。"
                action={
