...
export function UsersPanel() {
  ...
  const pageUsers = useMemo(() => visibleUsers.slice((page - 1) * pageSize, page * pageSize), [page, visibleUsers]);
  ...
  const openRoles = (user: UserFixture) => { setRoleTarget(user); setDraftRoles([...user.roles]); };
  const saveRoles = () => {
  ...
  const toggleRole = (role: string) => setDraftRoles((items) => items.includes(role) ? items.filter((item) => item !== role) : [...items, role]);
  ...
  return (
    <div data-pattern="collection-composition" className="flex flex-col gap-5">
      ...
      {scenario === "error" ? null : scenario === "loading" ? (
        ...
        <>
          <Table bordered>
            ...
            <TableBody>{pageUsers.map((user) => (
              <TableRow key={user.id}>
                <TableCell className="min-w-64 whitespace-normal"><div className="flex flex-wrap items-center gap-2"><span className="type-weight-semibold">{user.displayName}</span>{user.current ? <Tag color="primary">当前管理员</Tag> : null}</div><Text size="xs" tone="muted" family="mono" className="mt-1">{user.username} · {user.id}</Text><Text size="xs" tone="muted">{user.email}</Text></TableCell>
                ...
                <TableCell><div className="flex min-w-max flex-nowrap items-center justify-end gap-1"><IconButton label={`管理 ${user.displayName} 角色`} variant="ghost" icon={<UserRoundCog />} onClick={() => openRoles(user)} /><IconButton label={`重置 ${user.displayName} 密码`} variant="ghost" icon={<KeyRound />} disabled={user.current} onClick={() => { setPassword(""); setPasswordTarget(user); }} /><ConfirmAction label={user.status === "active" ? "暂停" : "启用"} icon={user.status === "active" ? <Lock /> : <Unlock />} title={`${user.status === "active" ? "暂停" : "启用"}“${user.displayName}”？`} description={user.status === "active" ? "暂停后该账户不能继续登录。" : "启用后该账户可重新参与认证。"} confirmText={user.status === "active" ? "确认暂停" : "确认启用"} color={user.status === "active" ? "error" : "primary"} disabled={user.current} onConfirm={() => toggleStatus(user)} /><ConfirmAction label="删除" icon={<Trash2 />} title={`删除“${user.displayName}”？`} description="真实产品会要求 Sudo/强认证，并删除或解绑该账户的相关身份数据。" confirmText="确认删除" disabled={user.current} onConfirm={() => { setUsers((items) => items.filter((item) => item.id !== user.id)); setStatus(`用户“${user.displayName}”已删除（Showcase 模拟）。`); }} /></div></TableCell>
              </TableRow>
            ))}</TableBody>
          </Table>
      ...
      <Modal open={createOpen} title="添加用户" description="创建新的本地身份账户。真实产品仍由服务端执行用户名、密码和权限校验。" onOpenChange={setCreateOpen} footer={<><Button onClick={() => setCreateOpen(false)}>取消</Button><Button form="system-create-user" type="submit" variant="solid" color="primary">创建用户</Button></>}>
        <form id="system-create-user" className="flex flex-col gap-5" onSubmit={createUser}><FormField label="用户名" required><Input value={username} onChange={(event) => setUsername(event.target.value)} /></FormField><FormField label="显示名称"><Input value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></FormField><FormField label="邮箱" required><Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></FormField></form>
      </Modal>
      <Modal open={Boolean(roleTarget)} title={roleTarget ? `管理 ${roleTarget.displayName} 的角色` : "管理角色"} description="角色变更属于高权限操作；真实产品会要求 Sudo/强认证。" onOpenChange={(next) => { if (!next) setRoleTarget(null); }} onOk={saveRoles} okText="保存角色"><CheckboxGroup label="可分配角色">{assignableRoles.map((role) => <Checkbox key={role} label={role} checked={draftRoles.includes(role)} onChange={() => toggleRole(role)} />)}</CheckboxGroup></Modal>
      <Modal open={Boolean(passwordTarget)} title={passwordTarget ? `重置 ${passwordTarget.displayName} 的密码` : "重置密码"} description="真实产品会要求 Sudo/强认证并应用完整密码策略。" onOpenChange={(next) => { if (!next) setPasswordTarget(null); }} onOk={() => { if (!passwordTarget || password.length < 12) return; setStatus(`已重置“${passwordTarget.displayName}”的密码（Showcase 模拟）。`); setPasswordTarget(null); }} okText="确认重置" okButtonProps={{ disabled: password.length < 12 }}><FormField label="新密码" hint="至少 12 个字符" required><Input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" /></FormField></Modal>
    </div>
  );
}
