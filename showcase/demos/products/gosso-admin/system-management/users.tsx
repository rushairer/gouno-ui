          ...
          <Table bordered>
            ...
            <TableBody>{pageUsers.map((user) => (
              <TableRow key={user.id}>
                <TableCell className="min-w-64 whitespace-normal"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold">{user.displayName}</span>{user.current ? <Tag color="primary">当前管理员</Tag> : null}</div><Text size="xs" tone="muted" className="mt-1 font-mono">{user.username} · {user.id}</Text><Text size="xs" tone="muted">{user.email}</Text></TableCell>
