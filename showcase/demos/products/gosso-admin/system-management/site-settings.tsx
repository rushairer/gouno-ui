...
const scenarioOptions = [
  { value: "data", label: "已载入" },
  { value: "loading", label: "加载中" },
  { value: "error", label: "加载失败" },
] as const;

function LoadingSiteSettings() {
  return (
    <Card padding="base" role="status" aria-label="站点设置加载中">
      <Text size="sm" tone="muted">正在加载站点设置…</Text>
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="space-y-4">{Array.from({ length: 6 }, (_, index) => <Skeleton key={index} className={index === 4 ? "h-24 w-full" : "h-10 w-full"} />)}</div>
  ...
  return (
    <div className="flex flex-col gap-5">
      <FixtureBanner
        ...
        note="站点设置 Fixture 先覆盖读取生命周期；dirty/save/save-error 与 recent-MFA/Sudo 写操作在业务态层独立建模。"
        controls={<Segmented<FixtureScenario> aria-label="站点设置 Fixture 状态" options={scenarioOptions} value={scenario} onChange={changeScenario} block />}
      />
