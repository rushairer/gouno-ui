...
import { Alert, Button, Card, Heading, Tag, Text } from "../../../../src/core";
...
export function SystemStatusPanel() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <ManagementSectionHeader title="系统状态" description="查看身份服务健康探针、关键依赖、OpenID Connect 发现信息和安全策略摘要。" actions={<Button icon={<RefreshCw />} onClick={() => setRefreshCount((count) => count + 1)}>刷新状态</Button>} />
      ...
      <Card padding="base">
        <div className="mb-4 flex items-start justify-between gap-3"><div><Heading level={3} className="text-base">基础设施健康</Heading><Text size="sm" tone="muted" className="mt-1">数据库与 Redis 是 GOSSO 会话、锁和身份数据的关键依赖。</Text></div><Button size="small" onClick={() => setDegraded((value) => !value)}>切换故障 Fixture</Button></div>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="flex items-center gap-3 rounded-lg border p-4"><span className={`flex size-10 items-center justify-center rounded-lg ${degraded ? "bg-danger-subtle text-destructive" : "bg-success-subtle text-success"}`}><Server aria-hidden="true" className="size-5" /></span><div className="min-w-0 flex-1"><div className="font-medium">Redis</div><Text size="xs" tone="muted">会话缓存与分布式锁</Text></div><Tag color={degraded ? "error" : "success"}>{degraded ? "异常" : "正常"}</Tag></div>
        </div>
      </Card>
      <DefinitionCard title="OpenID Connect 配置" rows={oidcRows} />
      <Card padding="base"><div className="mb-4 flex items-center gap-2"><ShieldCheck aria-hidden="true" className="size-5 text-primary" /><Heading level={3} className="text-base">支持能力</Heading></div><div className="grid gap-4 md:grid-cols-2"><div><Text size="xs" tone="muted" className="mb-2">Scopes</Text><div className="flex flex-wrap gap-2">{["openid", "profile", "email", "admin"].map((item) => <Tag key={item} color={item === "admin" ? "warning" : "primary"}>{item}</Tag>)}</div></div><div><Text size="xs" tone="muted" className="mb-2">Grant Types</Text><div className="flex flex-wrap gap-2">{["authorization_code", "refresh_token", "client_credentials", "device_code"].map((item) => <Tag key={item}>{item}</Tag>)}</div></div></div></Card>
